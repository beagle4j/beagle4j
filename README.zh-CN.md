...
| **慢查询** | ✅ | 实测耗时，不是估算 |
| 无效的预加载 | 🚧 | 查出来的关联根本没人用 |
| `@Transactional` 静默失效 | 🚧 | 自调用问题，**运行时确证**，不是静态分析瞎猜 |
...
没有开启会话的时候，每条 SQL 只多一次空指针判断，所以在关闭状态下留着它没有负担。
...
> **性能数据尚未实测。** JMH 基准测试是路线图上的下一项。在数字出来之前，
> 上面这些请当作设计意图看待，而不是已验证的结论。
...
控制台报告器和 Spring Boot Starter 都已可用，有 31 个测试覆盖，包含针对真实数据库的
端到端检测。接下来是事务反模式检测、JMH 基准测试和 HTML 报告。
